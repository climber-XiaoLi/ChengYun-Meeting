...
{
    ui->setupUi(this);
    this->setWindowTitle("澄云会议 注册&登录");
...
void LoginDialog::on_pb_commit_clicked()
{
    ...
    //1、校验手机号：正则表达式 校验手机号
    QRegularExpression reg("^1[3-8]\\d{6,9}$");
    bool res = reg.match(strTel).hasMatch();
    if(!res){
    ...
    if(strPassword.length() > 20){
        QMessageBox::about(this, "提示", "密码过长，长度不能超过20位");
        return;
    }
...
void LoginDialog::on_pb_commit_register_clicked()
{
    ...
    //1、校验手机号：正则表达式 校验手机号
    QRegularExpression reg("^1[3-9][0-9]{6,9}$");
    bool res = reg.match(strTel).hasMatch();
    if(!res){
        QMessageBox::about(this, "提示", "手机号格式错误，8-11位手机号");
        return;
